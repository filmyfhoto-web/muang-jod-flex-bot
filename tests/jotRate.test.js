...
/* รายละเอียดที่ร้านพิมพ์ ต้องขึ้นใบเสร็จ ไม่ใช่หายไปเพราะมีขนาด
 *
 * ร้านบอกว่า "พิมพ์รายละเอียดงานก็ไม่ขึ้นในใบเสร็จนะ" — ของเดิมเลือกอย่างใด
 * อย่างหนึ่ง มีขนาดเมื่อไหร่รายละเอียดหายทันที ทั้งที่คนละเรื่องกัน
 */
test('บรรทัดย่อยบนใบเสร็จ มีทั้งขนาดและรายละเอียด', () => {
  // เคสที่ร้านเจอ: กรอกครบทั้งชื่อ รายละเอียด และขนาด
  assert.equal(
    MJRate.subLine('160 × 300 ซม.', 'ตอกตาไก่ 4 มุม เคลือบด้าน', 'ป้ายไวนิล รพ.สต.'),
    '160 × 300 ซม. · ตอกตาไก่ 4 มุม เคลือบด้าน'
  );

  // มีอย่างเดียวก็ได้อย่างเดียว
  assert.equal(MJRate.subLine('160 × 300 ซม.', '', 'ป้าย'), '160 × 300 ซม.');
  assert.equal(MJRate.subLine(null, 'เคลือบด้าน', 'ป้าย'), 'เคลือบด้าน');
  // ไม่มีอะไรเลยต้องเป็น null ไม่ใช่สตริงว่าง (คอลัมน์ในฐานข้อมูลควรเป็น null)
  assert.equal(MJRate.subLine(null, '', 'ป้าย'), null);
  assert.equal(MJRate.subLine(), null);

  // รายละเอียดที่พิมพ์ซ้ำกับชื่องาน ไม่ต้องขึ้นสองรอบ
  assert.equal(MJRate.subLine(null, 'ป้ายไวนิล', 'ป้ายไวนิล'), null);
  assert.equal(MJRate.subLine('60 × 40 ซม.', 'ป้ายไวนิล', 'ป้ายไวนิล'), '60 × 40 ซม.');
  // ช่องว่างหัวท้ายไม่ทำให้กลายเป็นคนละคำ
  assert.equal(MJRate.subLine(null, '  ป้ายไวนิล  ', 'ป้ายไวนิล'), null);
