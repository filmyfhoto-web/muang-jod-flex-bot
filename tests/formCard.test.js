...
// The card people actually see in chat: the form's shape, white on blue, as
// the design asks. Nothing here can be rendered in a test, so what is guarded
// is what silently breaks — the send path repainting it dark, a card LINE
// would reject, or a row that leads nowhere.
...
test('the card survives the send path with its white background intact', () => {
  // Every bubble gets the dark surface stamped on it on the way out. This one
  // sets its own first, and that has to win — otherwise the design arrives as
