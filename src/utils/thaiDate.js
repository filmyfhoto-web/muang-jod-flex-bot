...
function dueMatchers(today) {
  return [
    ...
    /* "รับไปแล้ว" / "ส่งแล้ว" — ของออกจากร้านไปแล้ว วันนัดคือวันนี้
     *
     * ร้านตอบคำถาม "นัดรับวันไหนคะ?" ว่า "รับไปแล้ว จ่ายเงินแล้ว" แล้วม่วงตอบ
     * ว่าไม่เข้าใจ ทั้งที่เป็นคำตอบที่ตรงคำถามที่สุด: งานจบแล้ว
     *
     * ต้องไม่มีตัวเลขตามมา ไม่งั้นจะไปทับ "รับมาแล้ว 300" ซึ่งเป็นเงินมัดจำ
     */
    (t) => {
      const m = /^\s*(?:ไป|มา)?แล้ว(?!\s*\d)/.exec(t);
      return m ? [today, m[0]] : null;
    },
    (t) => {
