...
function iconChip(emoji, { size = '30px', bg = BLUE_SOFT, text = 'sm', color = BLUE_DEEP } = {}) {
  return {
    type: 'box',
    layout: 'vertical',
    width: size,
...
function cardHead(title, subtitle, emoji, mascot) {
  const contents = [
    iconChip(emoji, { size: '46px', bg: BLUE, text: 'xl', color: CARD }),
    {
      type: 'box',
      layout: 'vertical',
      flex: 1,
...
function formBubble(formUrl) {
  ...
  const body = [
    ...
    ...(open
      ? [{ type: 'text', text: 'แตะช่องไหนก็ได้ เพื่อเปิดฟอร์มกรอกค่ะ 💜', size: 'xxs', color: BLUE_DEEP }]
      : []),
