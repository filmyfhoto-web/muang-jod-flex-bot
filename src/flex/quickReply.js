...
// งานที่ร้านทำบ่อยที่สุด กดแล้วม่วงเริ่มถามรายละเอียดของงานชนิดนั้นให้ในแชตเลย
// ตอบไปทีละข้อจนครบ แล้วออกใบเสร็จจากการ์ดที่เด้งมาได้
//
...
// ยาว 25 ซึ่ง LINE ไม่รับ ป้ายจึงสั้นกว่าชื่อที่ส่งไปให้ม่วงอ่าน คนละช่องกัน
...
// ปุ่มงานไม่ต้องพึ่ง LIFF อีกแล้ว (กดแล้วม่วงถามต่อในแชต) ชุดนี้จึงเหลือไว้
// เป็นตาข่ายกันแถบว่างเท่านั้น เหตุผลอยู่ที่ withQuickReply ข้างล่าง
const FALLBACK = [
...
/* ปุ่มงานกดแล้วม่วงถามต่อในแชต ไม่ใช่เปิดฟอร์ม
 *
 * ร้านบอกว่า "ตรงปุ่มกดให้เป็นบอทตอบเหมือนเดิม แล้วค่อยกดเข้าไปแก้ไขทีหลัง"
 * ของเดิมเป็นลิงก์เปิดฟอร์มจดด่วน แปลว่าจะจดงานทีต้องเด้งออกจากแชตไปหน้าเว็บก่อน
 * ทุกครั้ง ตอนนี้ม่วงถามทีละข้อในแชตได้แล้ว ฟอร์มจึงเหลือไว้ตอนอยากแก้หลายช่อง
 *
 * ผลพลอยได้: ปุ่มงานใช้ได้แม้ยังไม่ได้ตั้ง LIFF ส่วน "หมวดงาน" ยังต้องใช้ เพราะ
 * มันคือหน้าเว็บจริง ๆ ไม่ใช่บทสนทนา
 */
export function defaultItems() {
  const jobs = QUICK_JOBS.map((j) => ({
    ...j,
    action: 'quick_job',
    params: { name: j.name },
    text: `จด${j.name}`,
    ...
    items: items.slice(0, 13).map((item) => ({
      ...
      action: item.uri
        ...
        : {
            type: 'postback',
            label: item.label,
            data: new URLSearchParams({ action: item.action, ...(item.params || {}) }).toString(),
