...
อัดเสียงส่งเข้าแชทได้เลย เช่นพูดว่า *"10 กันยา ไก่ทอดน้ำปลา 278"*
บอตจะถอดเสียง แล้วส่งข้อความนั้นเข้าทางเดิมที่ใช้กับข้อความพิมพ์ — คำสั่งอะไรที่พิมพ์ได้
ก็พูดได้หมด ทั้ง "สรุปวันนี้" "ค้างรับ" ไม่ใช่แค่บันทึกงาน

คำตอบจะขึ้น `🎤 ได้ยินว่า "…"` กำกับไว้ด้วยเสมอ เพราะถ้าถอดเสียงผิดจะได้เห็นทันที
ว่าผิดตรงไหน ไม่ใช่เห็นแค่ตัวเลขที่ไม่ตรง

Claude ฟังเสียงไม่ได้ ส่วนนี้จึงต้องพึ่งเจ้าอื่น ตั้งคีย์ตัวใดตัวหนึ่ง (ถ้าใส่ทั้งคู่ OpenAI ชนะ):

| ตัวแปร | ใช้ตัวไหน | โมเดลเริ่มต้น (เปลี่ยนที่ `STT_MODEL`) |
|---|---|---|
| `OPENAI_API_KEY` | OpenAI | `gpt-4o-mini-transcribe` |
| `GEMINI_API_KEY` | Gemini | `gemini-flash-latest` |

`MAX_VOICE_SECONDS` จำกัดความยาว (ค่าเริ่มต้น 120 วินาที) เสียงที่ยาวกว่านั้นถูกปฏิเสธ
ตั้งแต่ก่อนดาวน์โหลด จะได้ไม่เสียเงินฟรี · ดูสถานะได้ที่ `/health` ฟิลด์ `voice`

### ระบุวันที่ย้อนหลัง

ขึ้นต้นข้อความ (หรือเสียง) ด้วยวันที่ได้เลย ระบบจะตัดออกจากชื่อรายการให้เอง:

`10 กันยา` · `10 ก.ย. 68` · `10/9/2568` · `เมื่อวาน` · `วานซืน` · `3 วันก่อน` · `วันนี้`

ปีที่ไม่ได้บอกจะหมายถึงวันที่ **เพิ่งผ่านมา** — พูดว่า "15 ธันวา" ตอนเดือนกันยายน
คือธันวาปีที่แล้ว ไม่ใช่อีกสามเดือนข้างหน้า (แต่ล้ำหน้าไม่เกิน 7 วันถือว่าพูดพลาด ยังนับเป็นปีนี้)
