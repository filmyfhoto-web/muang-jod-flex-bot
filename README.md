...
## ⌨️ ปุ่มลัดเหนือช่องพิมพ์ (Quick Reply)

ริชเมนูใช้ได้ดีบนมือถือ แต่บนคอมมันหุบอยู่ ต้องกดเปิดทุกครั้ง — แถบปุ่มลัดลอยอยู่
เหนือช่องพิมพ์ตลอด กดได้ทันทีทั้งสองที่

📝 บันทึกงาน · 📊 สรุปวันนี้ · 💰 ค้างรับ · 🕘 ล่าสุด · 🧾 ออกบิล · ⏰ ตั้งเตือน ·
📈 รายงาน · ❓ ช่วยเหลือ

แก้รายการปุ่มได้ที่ `src/flex/quickReply.js` ที่เดียว แถบนี้ถูกแนบให้อัตโนมัติที่
ทางส่ง (`lineService`) กับ **ข้อความใบสุดท้าย** ของทุกครั้งที่บอทตอบ — LINE โชว์
แค่ใบสุดท้ายอยู่แล้ว ข้อความใบไหนตั้ง `quickReply` มาเองก็ไม่ถูกทับ

> LINE จำกัด 13 ปุ่ม ป้ายละ 20 ตัวอักษร — มีเทสต์คุมทั้งสองข้อ และคุมว่าทุกปุ่ม
> ต้องมี handler จริง ทั้งตอนกดและตอนข้อความที่มันพิมพ์แทนเราถูกส่งเข้ามา
...
**สองธีม** — ค่าเริ่มต้นเป็นการ์ดขาวปุ่มน้ำเงิน **ชุดเดียวกับการ์ดในแชต**
(`src/flex/theme.js`) เพราะหน้านี้เปิดจากการ์ด เปิดมาเจอคนละโทนคือสะดุด
`?theme=night` ได้โทนกรมท่ากลางคืน ทุกสีอ้าง `var()` ทั้งไฟล์ สลับธีมจึงเป็น
แค่คลาสเดียวบน `body` (มีเทสต์กันไม่ให้มีสีดิบหลุดออกนอกบล็อก token และคุมว่า
ค่าเริ่มต้นต้องตรงกับ `theme.js`)
