...
| 📎 แนบสลิป/หลักฐาน | ส่งรูป/ไฟล์ — อ่านสลิปหรือใบสั่งงานให้ ถ้าอ่านไม่ได้ก็แนบเข้ากับงานล่าสุด |
...
4. ทำแบบเดียวกันกับไฟล์ที่เหลือใน `supabase/migrations/` ตามลำดับเลข (003 → 007)
...
ส่งรูปเข้าแชทได้เลย ม่วงจดจะดูก่อนว่าเป็นอะไร แล้วทำต่างกัน:

| รูปที่ส่ง | ม่วงจดทำอะไร |
|-----------|---------------|
| **สลิป / ใบเสร็จ** (จ่ายแล้ว) | บันทึกเป็นงานใหม่ทันที ทำเครื่องหมายว่ารับเงินแล้ว + แนบรูปไว้ |
| **ใบสั่งงาน / แบบงาน / ใบเสนอราคา** | อ่านชื่องาน ลูกค้า ขนาด จำนวน ออกมาเป็น **ร่าง** ให้ตรวจก่อน กด "✅ บันทึกงาน" ถึงจะเก็บ แล้วค่อยแนบรูปตอนนั้น |
| **รูปอื่น** (เช่น รูปงานที่ทำเสร็จ) | แนบเป็นหลักฐานของงานล่าสุดเหมือนเดิม |

เอกสารสั่งงานส่วนใหญ่ไม่มีราคา — ถ้าอ่านมาแล้วราคาเป็น 0 บอตจะขอราคา
พิมพ์ตัวเลขเปล่า ๆ กลับไป เช่น `1500` แล้วม่วงจดจะใส่ให้ในร่างเดิม
(ถ้าเป็นงานคิดตามพื้นที่ ตัวเลขนั้นคือ **ยอดรวม** ระบบจะหารกลับเป็นราคาต่อหน่วยให้เอง)

ทั้งหมดนี้ต้องมี `ANTHROPIC_API_KEY` (โมเดลปรับได้ที่ `VISION_MODEL`) — ถ้าไม่ได้ตั้ง
บอตจะกลับไปแค่แนบไฟล์เฉย ๆ เหมือนเดิม ดูได้จาก `/health` ที่ฟิลด์ `vision`

### ระบุวันที่ย้อนหลัง

ขึ้นต้นข้อความด้วยวันที่ได้เลย ระบบจะตัดออกจากชื่อรายการให้เอง:

`10 กันยา` · `10 ก.ย. 68` · `10/9/2568` · `เมื่อวาน` · `วานซืน` · `3 วันก่อน` · `วันนี้`

ปีที่ไม่ได้บอกจะหมายถึงวันที่ **เพิ่งผ่านมา** — พิมพ์ "15 ธันวา" ตอนเดือนกันยายน
คือธันวาปีที่แล้ว ไม่ใช่อีกสามเดือนข้างหน้า (แต่ล้ำหน้าไม่เกิน 7 วันถือว่าพิมพ์พลาด ยังนับเป็นปีนี้)

### วันนัดรับงาน

งานที่จดวันนี้อาจนัดรับอีกสิบวันข้างหน้า — คนละวันกับวันที่จด บอกวันนัดมาได้เลย
โดยมีคำกำกับ:

`นัดรับ 15 ก.ย.` · `รับก่อน 20/9` · `วันรับ 1 ต.ค.` · `นัดส่ง พรุ่งนี้` · `ส่งอีก 3 วัน`

การ์ดจะขึ้น `📅 นัดรับ 20 ก.ย. 2569 · อีก 10 วัน` และนับถอยหลังให้ — ใกล้ถึง
(วันนี้/พรุ่งนี้) เป็นสีส้ม เลยกำหนดแล้วเป็นสีแดงพร้อมบอกว่าเลยมากี่วัน

ต้องมีคำกำกับเสมอ เพราะ `รับมาแล้ว 300` คือเงินมัดจำ ไม่ใช่วันที่ ระบบจึงรับ
เฉพาะวันที่ที่ชัดเจน (มีชื่อเดือน / มีขีดคั่น / พรุ่งนี้-มะรืน) ต่อท้ายคำกำกับเท่านั้น

ปีที่ไม่ได้บอกของวันนัด = วันที่**กำลังจะมาถึง** (กลับด้านกับวันที่จด ซึ่งหมายถึง
วันที่เพิ่งผ่านมา) พูด "นัดรับ 5 ม.ค." ตอนเดือนกันยาคือมกราปีหน้า

ในฟอร์มจดด่วนมีช่อง **วันรับงาน** แยกจาก **วันที่จด** และแก้ทีหลังได้ในหน้าแก้ไข

> ⚠️ ต้องรัน migration `007_due_date.sql` ก่อน ไม่งั้นวันนัดจะไม่ถูกเก็บ
> (งานยังบันทึกได้ตามปกติ — ระบบตั้งใจใส่วันนัดหลังบันทึกแถวเสร็จแล้ว
> งานจึงไม่หายเพราะคอลัมน์ที่ยังไม่มี)

### ลูกค้ารายเดียว หลายงาน

การ์ด "บันทึกสำเร็จ" มีปุ่ม **➕ เพิ่มงานอีก** — กดแล้วพิมพ์เฉพาะงานถัดไปได้เลย
ไม่ต้องพิมพ์ชื่อลูกค้าซ้ำ ระบบจำชื่อไว้ให้จนกว่าจะพิมพ์ชื่อใหม่มาเอง
(ชื่อในข้อความชนะเสมอ — เผื่อเปลี่ยนไปคุยกับลูกค้าคนถัดไปโดยไม่ได้กดปุ่ม)

**ส่งรูปหน้างานพร้อมกันได้** — พิมพ์งาน → การ์ดตรวจสอบขึ้น → ส่งรูปตามเข้าไป
บอทจะเก็บรูปไว้ให้ แล้วแนบตอนกด "✅ บันทึกงาน" ไม่ใช่ไปแนบกับงานเก่าที่บันทึกไว้ก่อนหน้า
(รูปถูกอัปโหลดตอนกดบันทึกเท่านั้น ร่างที่ยกเลิกจึงไม่กินที่เก็บ)

### ออกใบเสร็จ แล้วส่งให้ลูกค้าในไลน์

"ออกบิล" คือการออก **ใบเสร็จรับเงิน** ออกได้สองแบบ:

| ออกจากไหน | ได้อะไร |
|---|---|
| ปุ่ม 🧾 **ออกบิล** | เลือกลูกค้า → รวมงานที่ยังไม่ได้ออกบิลของลูกค้ารายนั้นเป็นใบเดียว |
| ปุ่ม 🧾 **ออกใบเสร็จ** บนการ์ดงาน | ออกให้งานนั้นงานเดียว ส่งมอบพร้อมงานได้เลย |

งานหนึ่งอยู่ได้บิลเดียว กดออกซ้ำจะได้บิลเดิมกลับมา ไม่ออกใหม่ทับ

**ส่งให้ลูกค้า** — บนการ์ดบิลและการ์ดใบเสร็จมีปุ่ม **📤 ส่งให้ลูกค้า** กดแล้ว LINE
จะเปิดหน้าเลือกแชตของร้านเอง พร้อมข้อความที่เขียนไว้ให้แล้ว:

```
ใบเสร็จรับเงิน 🧾
เลขที่ MJ-B-20260910-0001
ลูกค้า: รพ.สต.บ้านชี
ยอด ฿1,200

https://<โดเมนบอต>/r/<share_token>
```

ร้านเลือกแชตลูกค้าแล้วกดส่งเอง — บอทจึงไม่ต้องรู้ว่าลูกค้าเป็นใครใน LINE
และไม่เสียโควตา push ลูกค้ากดลิงก์เปิดใบเสร็จได้โดยไม่ต้องเป็นเพื่อนกับบอท

ปุ่มทั้งสองจะไม่ขึ้นเลยถ้ายังไม่มีโดเมนสาธารณะ (`PUBLIC_BASE_URL` หรือ Render
ตั้งให้เอง) เพราะไม่มีลิงก์ให้ส่ง
...
→ ลูกค้า รพสตบ้านชี · 160 × 300 ซม. · 792 บาท
   (คิดจาก 4.8 ตร.ม. × 165 — วิธีคิดอยู่ในหมายเหตุ ไม่ขึ้นใบเสร็จ)
```
...
| หลายผืน | `160x300 2 ผืน ตรมละ 165` → 2 ผืน ผืนละ 792 = 1,584 บาท |
| บอกพื้นที่มาเลย | `ไวนิล 4.8 ตร.ม. ตรมละ 165` |

**เรตต่อตารางเมตรเป็นวิธีคิดของร้าน ไม่ใช่ของลูกค้า** รายการที่บันทึกจึงเก็บเป็น
`จำนวน = จำนวนชิ้น` และ `ราคาต่อหน่วย = ราคาต่อชิ้นที่คิดได้` ใบเสร็จเห็นแค่
`ไวนิล 160 × 300 ซม. — 792` ไม่มีเรตและไม่มี "ตร.ม." อยู่บนนั้น

ส่วนวิธีคิด (`ไวนิล: 4.8 ตร.ม. × 165 = 792`) ไปอยู่ใน **หมายเหตุ** ของงาน ซึ่งขึ้น
เฉพาะการ์ดงานในแชตของร้าน ไม่ขึ้นการ์ดใบเสร็จ บิล หรือหน้าใบเสร็จสาธารณะ (`/r/…`)
ฟอร์ม "จดด่วน" ก็ทำแบบเดียวกัน งานเดียวกันจึงหน้าตาเหมือนกันไม่ว่าจะจดมาทางไหน
...
- `w` / `h` / `unit` คือ **กว้าง / ยาว / หน่วย** แยกช่องกัน ไม่ต้องพิมพ์ `x` ให้ระบบเดา
- `rate` คือ "ตรมละ" — **วิธีคิดของร้าน ไม่ขึ้นใบเสร็จ** ถ้ากรอกไว้พร้อมขนาด ระบบคิด
  `พื้นที่ × เรต` แล้วเติมลงช่องราคาให้เป็นตัวตั้งต้น พร้อมป้าย 🔒 ร้านเห็นคนเดียว
- `price` คือ **ราคาที่ร้านตั้งเอง** พิมพ์ทับได้เสมอ พอพิมพ์แล้ว (`priceManual`)
  ของร้านชนะตลอด เรตจะเปลี่ยนทีหลังก็ไม่ย้อนกลับไปทับ — อันนี้แหละที่ขึ้นใบเสร็จ
...
คุมไม่ให้สองที่หลุดจากกัน ในฟอร์มมีช่องหน่วยให้เลือกอยู่แล้ว กติกานี้จึงใช้กับ
ทางแชตเป็นหลัก
...
## ✂️ ปลั๊กอิน Adobe: นัดพอน Nesting Cut

ปลั๊กอิน Illustrator + Photoshop สำหรับงานสติ๊กเกอร์และป้าย มี 8 เครื่องมือ: **จัดวางดวง** ตามเส้นไดคัทจริง · **ไดคัทต่อเนื่อง** · **ไดคัทตามรูป** (รูป JPG-PNG/เวกเตอร์) · **ทำมุมโค้ง** ตามดอกกัด CNC · **หาเส้นกลาง** วางไฟ LED · **เส้นบอกขนาด** ตามสเกล · **รันนัมเบอร์** · **ปุ่มลัด** จัดวางในปุ่มเดียว — สร้างชีตบนอาร์ตบอร์ดใหม่ แยกเลเยอร์งานพิมพ์/เส้นไดคัท ใส่มาร์กและหัวงาน
